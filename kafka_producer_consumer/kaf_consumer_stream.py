...
# from pyspark.sql.functions import from_json, col, explode, to_timestamp, year, month
# from pyspark.sql.types import StructField, StructType, StringType, ArrayType, IntegerType, TimestampType

# # Dem so lan xuat hien cua tung tags


# def count_tags(df_parsed):
#     df_tags_exploded = df_parsed.select(explode("tags").alias("tag"))
#     df_tag_counts = df_tags_exploded.groupBy("tag").count()

#     df_tag_counts.writeStream \
...
#     .awaitTermination()
# # .option("es.nodes.wan.only", "true") \
# count_tags(df_parsed)

# # df_parsed.writeStream \
# #     .format("org.elasticsearch.spark.sql") \
# #     .option("es.nodes", "elasticsearch_final") \
# #     .option("es.port", "9200") \
# #     .option("es.resource", "test_questions_stream") \
# #     .option("es.nodes.wan.only", "true") \
# #     .outputMode("append") \
# #     .start() \
# #     .awaitTermination()
